...
export class SolicitudInicialRepositoryAdapter
  implements SolicitudInicialRepositoryPort
{
  ...
  async updateSolicitudInicial(
    ...
    try {
      ...
      if (result.rows.length === 0) {
        throw new Error(
          `Solicitud inicial con ID ${solicitudInicial.getId()} no encontrada`
        );
      }
      ...
      const solicitudActualizada = await this.getSolicitudInicialById(
        solicitudInicial.getId()
      );
   ...
   * @returns Promise<SolicitudInicial> - La solicitud inicial actualizada.
   */
  async updateSolicitudInicialAprobaciónRechazo(
    solicitudInicial: SolicitudInicial,
    cliente: Cliente
  ): Promise<SolicitudInicial> {
    const client = await pool.connect();
...
async getAllSolicitudesIniciales(): Promise<SolicitudInicial[]> {
   ...
   */
  async getSolicitudesInicialesByComercianteId(
   ...
   * @returns Promise<SolicitudInicial[]> - Array de solicitudes iniciales del cliente.
   */
  async getSolicitudesInicialesByClienteId(
    clienteId: number
  ): Promise<SolicitudInicial[]> {
    const query = `
            SELECT 
  ...
  private mapRowToSolicitudInicial(row: any): SolicitudInicial {
    const solicitud = new SolicitudInicial({
      ...
      estado: row.estado as "pendiente" | "aprobada" | "rechazada" | "expirada",
    ...
    if (row.comerciante_nombre && row.comerciante_apellido) {
      solicitud.setComercianteNombre(
        `${row.comerciante_nombre} ${row.comerciante_apellido}`
      );
    }
    ...
    solicitud.setEstadoSolicitudFormal(row.estado_solicitud_formal);
  solicitud.setEstadoCompra(row.estado_compra);
